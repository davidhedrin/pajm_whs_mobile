...
const PRDetail = () => {
  ...
  const handlePrAction = async ({ action, doc_id, level, remark, send_email }: PrPoActionProps) => {
  ...
  if (loading) return <View className='flex-1 justify-center items-center'
    style={{
      marginTop: rpm(-65)
    }}
  >
    ...
    >
      {
        dataPr ? <View style={{ paddingTop: rpm(16), paddingHorizontal: rpm(12) }}>
...

          {
            resCheckAprLevel && (
              resCheckAprLevel.show ? <View style={{ marginBottom: rpm(14) }}>
                ...
                <View style={{ marginBottom: rpm(14) }}>
                  <Input
                    value={remark}
        ...
        </View> : <View className='flex-1 justify-center items-center' style={{ marginTop: rpm(-20) }}>
          <Ionicons name="folder-open-outline" size={rf(38)} color="#9CA3AF" style={{ marginBottom: rpm(10) }} />
          ...
          <CText className="font-regular text-center" style={{ color: colors.textMuted, fontSize: rf(13) }}>
            Something wrong to find the data.
          </CText>
        </View>
      }
    </ScreenWrapper>
